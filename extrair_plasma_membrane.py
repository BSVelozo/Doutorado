...
def carregar_ids_plasma(tsv_file):
    ids = set()
    with open(tsv_file, 'r', encoding='utf-8') as f:
        ...
        try:
            ...
            print("[ERRO] Colunas 'Uniprot' ou 'Subcellular location' nao encontradas no cabecalho")
            return ids

        for line in f:
            cols = line.strip().split('\t')
            if len(cols) <= max(idx_uniprot, idx_subcell):
                continue
            subcell = cols[idx_subcell]
            if "Plasma membrane" in subcell:
                uniprot_id = cols[idx_uniprot].strip()
                if uniprot_id:
                    ids.add(uniprot_id)
    print(f"[INFO] Total de IDs UniProt com 'Plasma membrane': {len(ids)}")
    return ids
...
def filtrar_fasta_por_ids(fasta_in, fasta_out, ids_uniprot):
    with open(fasta_in, 'r', encoding='utf-8') as fin, open(fasta_out, 'w', encoding='utf-8') as fout:
        ...
        for line in fin:
            if line.startswith(">"):
                total_seq += 1
                # extrai o ID do header (primeira palavra apos >)
...
def main():
    ...
    ids_uniprot = carregar_ids_plasma(tsv_file)
    if not ids_uniprot:
