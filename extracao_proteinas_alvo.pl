...
my $output_file = "linhas_extracelular.csv";
...
while (my $line = <$in_fh>) {
    ...
    # Verifica se a célula da coluna "Localizations" contém "Extracellular"
    if (defined $columns[1] && $columns[1] =~ /Extracellular/i) {
...
print "Linhas com 'Extracellular' na coluna 'Localizations' foram salvas no arquivo '$output_file'.\n";
