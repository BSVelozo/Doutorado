...
def carregar_ids_predicted_membrane_plasma(tsv_file):
    ids = set()
    with open(tsv_file, 'r', encoding='utf-8') as f:
        ...
        try:
            ...
            idx_subcell = header_cols.index("subcellular location")
            idx_protein_class = header_cols.index("protein class")
        except ValueError:
            print("[ERRO] Colunas 'Uniprot', 'Subcellular location' ou 'Protein class' nao encontradas no cabecalho")
            return ids

        for line in f:
            cols = line.strip().split('\t')
            if len(cols) <= max(idx_uniprot, idx_subcell, idx_protein_class):
                continue
            subcell = cols[idx_subcell]
            protein_class = cols[idx_protein_class]
            if "Plasma membrane" in subcell and "Predicted membrane proteins" in protein_class:
                uniprot_id = cols[idx_uniprot].strip()
                if uniprot_id:
                    ids.add(uniprot_id)
    print(f"[INFO] Total de IDs UniProt com 'Predicted membrane proteins' + 'Plasma membrane': {len(ids)}")
    return ids
...
def main():
    ...
    ids_uniprot = carregar_ids_predicted_membrane_plasma(tsv_file)
    if not ids_uniprot:
